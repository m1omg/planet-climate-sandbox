...
* **Modern Earth holds 2 ppb of hydrogen, against an observed 0.55 ppm.** The two-year oxidative
...
* **Hydrogen was integrated explicitly and had no bound in `maxStep`** — the only reservoir with
  neither. Escape is very nearly first order in the amount present, so on an anoxic world it turns
...
* **Hydrogen was never in the snapshot.** `captureWorld` did not list it and `reset()` refills it
  ...
  the moment it was saved and reloaded — in every save slot, every export, and every drag of the
